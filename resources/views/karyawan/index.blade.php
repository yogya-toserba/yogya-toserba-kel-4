...
@push('styles')
    <style>
        ...
        .karyawan-info {
            background: rgba(255, 25 // Konfirmasi absensi

                $('#btnKonfirmasi').click(function() {
                        if ( !selectedKaryawan || !currentAction) return;

                        const keterangan=$('#keteranganText').val();
                        const url=currentAction === 'checkin' ? '{{ route('karyawan.checkin') }}' : '{{ route('karyawan.checkout') }}';

                        console.log('Debug - selectedKaryawan:', selectedKaryawan);
                        console.log('Debug - id_karyawan:', selectedKaryawan.id_karyawan);
